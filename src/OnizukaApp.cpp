...
OnizukaApp* OnizukaApp::pApp = 0;


OnizukaApp::OnizukaApp(HINSTANCE hinst)
    : pRender(0),
      LastUpdate(0),
...
int OnizukaApp::OnStartup(const char* args)
{
    OVR_UNUSED(args);
	...
	if(!simConnection.Initialize())
		return 1;

	if(!assetConnection.Initialize())
		return 1;
...
void OnizukaApp::OnMessage(const Message& msg)
{
...
void OnizukaApp::OnGamepad(float padLx, float padLy, float padRx, float padRy)
{
...
void OnizukaApp::OnMouseMove(int x, int y, int modifiers)
{
...
void OnizukaApp::OnKey(unsigned vk, bool down)
{
    switch (vk)
    {
    case 'Q':
...
void OnizukaApp::Render(const StereoEyeParams& stereo)
{
...
bool OnizukaApp::setupWindow()
{
    ...
    wc.cbWndExtra    = sizeof(OnizukaApp*);
    RegisterClass(&wc);
...
LRESULT CALLBACK OnizukaApp::systemWindowProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    if (msg == WM_NCCREATE)
        pApp->hWnd = hwnd;
...
void OnizukaApp::giveUsFocus(bool setFocus)
{
    if (setFocus)    
    {
...
LRESULT OnizukaApp::windowProc(UINT msg, WPARAM wp, LPARAM lp)
{
    switch (msg)
    {
    case WM_MOUSEMOVE:
